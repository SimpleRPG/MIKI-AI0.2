...
class WebLLMService {
  ...
  // How long we'll wait for the engine to hand back a stream iterator at all.
  private readonly STREAM_START_TIMEOUT_MS = 25000;
  // How long we'll wait between individual tokens before treating it as a frozen/hung generation.
  private readonly STREAM_TOKEN_TIMEOUT_MS = 20000;

  /** Races a promise against a timeout; rejects with `new Error(timeoutMessage)` if it's too slow. */
  private raceWithTimeout<T>(promise: Promise<T>, ms: number, timeoutMessage: string): Promise<T> {
  ...
  /**
   * Hard-stop: best-effort ask the engine to interrupt, then unbind it entirely so a
   * frozen/stuck generation can never block a future message. The next chat will
   * simply reload the model (a few seconds), which is far better than an infinite hang.
   */
  private async forceAbortEngine(): Promise<void> {
    ...
    try {
      if (typeof (engine as any).interruptGenerate === 'function') {
        await this.raceWithTimeout((engine as any).interruptGenerate(), 1500, 'INTERRUPT_TIMEOUT').catch(() => {});
      }
    } catch {}
    try {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
          try {
            ...
            if (this.engine) {
              ...
              try {
                // reload() reuses the existing engine instance. If that instance is
                // wedged from a previous frozen/stalled generation, reload() can hang
                // or silently fail the same way — so guard it with a timeout too.
                await this.raceWithTimeout(this.engine.reload(modelId), 30000, 'RELOAD_TIMEOUT');
              } catch (reloadErr: any) {
                console.warn('[WebLLM] reload() on existing engine failed/stalled, recreating engine fresh:', reloadErr);
                try { await (this.engine as any).unload?.(); } catch {}
                this.engine = null;
  ...
  public async interruptGenerate(): Promise<void> {
    ...
    try {
      if (typeof (this.engine as any).interruptGenerate === 'function') {
        // Give the engine's own interrupt a short window to work cleanly.
        const result = await Promise.race([
          (this.engine as any).interruptGenerate().then(() => 'ok'),
          new Promise((resolve) => setTimeout(() => resolve('timeout'), 1500)),
        ]).catch(() => 'error');

        if (result === 'ok') {
          return; // Engine stopped cleanly, no need to reload next time.
        }
        console.warn('[WebLLM] interruptGenerate did not resolve in time — forcing a hard engine reset.');
      }
    } catch (e) {
      console.warn('Error calling interruptGenerate on engine:', e);
    }

    // The engine's interrupt is unreliable (or unsupported) — nuke it so a stuck
    // generation can never silently block the next message.
    await this.forceAbortEngine();
  }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (attempt === 1) {
          ...
          // Guard the initial handshake with a timeout: if the engine never even
          // returns a stream iterator, we must not hang forever.
          const chunks = await this.raceWithTimeout(
            ...
            this.STREAM_START_TIMEOUT_MS,
            'STREAM_START_TIMEOUT'
          );

          let hasYielded = false;
          const iterator = (chunks as any)[Symbol.asyncIterator]();
          while (true) {
            // Guard every single token: if generation stalls mid-stream (a hung
            // WebGPU dispatch), don't block the UI forever — time it out.
            const step = await this.raceWithTimeout(
              iterator.next(),
              this.STREAM_TOKEN_TIMEOUT_MS,
        ...
        // A stalled/hung generation (no first token, or generation froze mid-stream).
        // The engine may be stuck holding VRAM/GPU work that never resolves — a plain
        // retry on the same engine instance would just hang again, so force a hard reset.
        if (errorMsg === 'STREAM_START_TIMEOUT' || errorMsg === 'STREAM_STALL_TIMEOUT') {
          await this.forceAbortEngine();
          lastError = new Error(
            '端末モデルの推論が一定時間応答しませんでした（フリーズ検出）。エンジンをリセットして再試行します。'
          );
          (lastError as any).name = 'InferenceStallError';
        }
        ...
        if (
          ...
          errorMsg === 'STREAM_START_TIMEOUT' ||
          errorMsg === 'STREAM_STALL_TIMEOUT'
        ) {
          try {
            const targetModel =
