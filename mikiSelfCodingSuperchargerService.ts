...
export interface UnitTestRunResult {
  ...
  passedCount: number;
  totalCount: number;
  // 実コードをサンドボックス実行して測定した結果かどうか。
  // false の場合、下記の値はオフライン等により計測できていない未測定値。
  measured?: boolean;
  executionError?: string | null;
  // 行トレースによるカバレッジ計測は未実装のため、計測できない場合は null。
  // 架空の数値を成功扱いの根拠にしないこと。
  coverage: {
    lines: number;
    branches: number;
...
export interface SelfImplementationResult {
  ...
  // 構文検証に加え、セキュリティ・クリーンコード・QA観点のレビュー(council-reviewと
  // 同一基準)に合格したかどうか。false の場合、構文が正しくても applied は false になる。
...
class MikiSelfCodingSuperchargerService {
  ...
  public async generateAndRunUnitTests(
    ...
    systemLogger.info(
      'SELF_IMPROVEMENT',
      `[TDDユニットテスト] ${res.passedCount}/${res.totalCount} パス (実測ベース、カバレッジ計測は未実装のためN/A)`
    );
    return res;
  }
