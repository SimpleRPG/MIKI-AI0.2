...
必要に応じてコードを埋めて使ってください！`,
  },
  {
    ...
    goal: 'MIKIの自律改善アーキテクチャの利点を教えて',
    response: `MIKIの自律改善アーキテクチャの主なメリットは以下の3点です：

1. **CORE統括と17分類**: COREが統括層として要求を正規ルーティングし、必要な分類のみを動的に連携させます。
2. **決定論的検証と安全性ゲート**: 変更候補は独立ワークスペースで事前検証・単一昇格ゲートを通過した安全な成果のみが反映されます。
3. **CORE Result統一**: UIからの全要求に一貫したrequestIdと状態追跡を提供し、透明性の高い実行結果を返却します。`,
  },
];
