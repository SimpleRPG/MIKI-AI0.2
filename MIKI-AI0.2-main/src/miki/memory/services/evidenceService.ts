...
import type { WebFetchMethod } from '../../../types';
...
export class EvidenceService {
  ...
  public recordCloudAiEvidence(input: { title: string; snippet: string; source: string; sourceId?: string; independenceClusterId?: string; metadata?: EvidenceRecord['metadata']; }): EvidenceRecord {
    ...
    const record: EvidenceRecord = {
      ...
      title: input.title.trim(),
      snippet: input.snippet.trim(),
      source: input.source.trim() || 'external_ai',
      source_id: input.sourceId,
  ...
  public registerExecutionClaim(evidenceId: string, statement: string): string | undefined {
    ...
    // Claim registration and evidence admission are separate responsibilities.
    // EXECUTION evidence remains DISCOVERED until the strict admission contract passes.
    this.save();
    return claim.claim_id;
  }
