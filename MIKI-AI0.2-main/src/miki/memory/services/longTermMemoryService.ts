...
class LongTermMemoryService {
  ...
  public async searchPipeline(
    ...
    try {
      queryEmbeddingResult = await deterministicRuntimeService.getEmbedding(query, undefined, 1500);
    } catch (e) {
  ...
  public async enrichMemoryEmbeddings(memories: MemoryItem[], maxItems = 5): Promise<boolean> {
    ...
    for (const mem of targets) {
      try {
        const emb = await deterministicRuntimeService.getEmbedding(mem.content, undefined, 2000);
        if (emb) {
