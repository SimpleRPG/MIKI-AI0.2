...
const QUESTION = /[？\?]|ですか|でしょうか|どうして|なぜ|何|どれ|どこ|いつ|誰|教えて/;
...
class JapaneseAnalysisService {
  ...
  public analyze(text: string): JapaneseAnalysisResult {
    return this.analyzeIntl(text) || this.analyzeDeterministic(text);
  }

  public analyzeIntl(text: string): JapaneseAnalysisResult | undefined {
    const normalizedText = normalize(text || '');
    if (!normalizedText) return this.buildResult(text || '', normalizedText, [], 'INTL_SEGMENTER');
    try {
      const Segmenter = (Intl as any)?.Segmenter;
      if (!Segmenter) return undefined;
      const segmenter = new Segmenter('ja', { granularity: 'word' });
      const tokens = Array.from(segmenter.segment(normalizedText)).map((part: any) => ({
      ...
      return this.buildResult(text || '', normalizedText, tokens, 'INTL_SEGMENTER');
    } catch {
      return undefined;
    }
  }

  public analyzeDeterministic(text: string): JapaneseAnalysisResult {
    const normalizedText = normalize(text || '');
    const tokens = normalizedText ? fallbackTokens(normalizedText).map(enrichToken) : [];
    return this.buildResult(text || '', normalizedText, tokens, 'DETERMINISTIC_FALLBACK');
  }

  private buildResult(text: string, normalizedText: string, tokens: JapaneseToken[], analyzer: JapaneseAnalysisResult['analyzer']): JapaneseAnalysisResult {
    const contentTokens = tokens.filter(t => t.kind === 'WORD' && t.normalized.length > 0)
      .map(t => t.normalized).filter((v, i, a) => a.indexOf(v) === i);
    const compact = normalizedText.replace(/\s+/g, '');
    const bigrams: string[] = [];
    for (let i = 0; i < compact.length - 1; i++) bigrams.push(compact.slice(i, i + 2));
    const dictionaryHits = tokens.filter(t => Boolean(t.dictionarySource)).length;
    const dictionarySources = [...new Set(tokens.map(t => t.dictionarySource).filter(Boolean) as string[])];
    return {
      text, tokens, contentTokens, particles: tokens.filter(t => t.kind === 'PARTICLE').map(t => t.surface),
      normalizedText, bigrams, hasQuestion: QUESTION.test(normalizedText), hasCorrection: CORRECTION.test(normalizedText),
      confidence: contentTokens.length >= 2 ? 'HIGH' : contentTokens.length ? 'MEDIUM' : 'LOW', analyzer,
      dictionaryHits, dictionarySources, semanticRoles: extractSemanticRoles(tokens)
    };
  }
