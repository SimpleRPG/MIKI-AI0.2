import { deterministicRuntimeService, DeterministicTeacherConfig } from '../../safety/services/deterministicRuntimeService';
import { storageService } from '../../../services/storageService';
import { systemLogger } from '../../../services/systemLogger';
import { nativeBackgroundService } from '../../execution/services/nativeBackgroundService';
...
class EmbeddingService {
   ...
   */
  public async checkAvailability(overrideConfig?: DeterministicTeacherConfig): Promise<{
    available: boolean;
    endpoint: string;
    type: string;
    dimensions?: number;
    ...
    if (this.lastCheckResult && now - this.lastCheckResult.timestamp < 30000 && !overrideConfig) {
      const cfg = deterministicRuntimeService.getActiveExternalConfig();
      return {
    ...
    const check = await deterministicRuntimeService.checkEmbeddingAvailability(overrideConfig);
  ...
  public async getQueryEmbedding(
    query: string,
    overrideConfig?: DeterministicTeacherConfig
  ): Promise<number[] | null> {
    ...
    try {
      const res = await deterministicRuntimeService.getEmbedding(trimmed, overrideConfig, 3000);
  ...
  public async ensureMemoryEmbedding(
    ...
    try {
      const res = await deterministicRuntimeService.getEmbedding(textToEmbed, overrideConfig, 3500);
  ...
  public async getStats(memories: MemoryItem[]): Promise<EmbeddingStats> {
    const config = deterministicRuntimeService.getActiveExternalConfig();
  ...
  public async checkHealth(
    ...
    if (
      ...
      try {
        const activeEndpoint = (overrideConfig ?? deterministicRuntimeService.getActiveExternalConfig()).endpoint;
