  ...
  private async queryDeviceModel(
    ...
    try {
      ...
      const stream = isNativeReady
        ? deterministicRuntimeService.streamDeterministicChat(messages, { temperature: 0.6, max_tokens: 380 })
        : deterministicRuntimeService.streamChat(messages, { temperature: 0.6, max_tokens: 380 });
  ...
  public async runDynamicConversationEvaluation(): Promise<DynamicEvaluationReport> {
    ...
    try {
      ...
      const report: DynamicEvaluationReport = {
        id: `dyn_${Date.now()}`,
        timestamp: Date.now(),
        modelName: 'Galaxy S25 Companion Core',
        engineType: false ? '退役生成ランタイム' : 'Non-LLM Core',
        turns,
