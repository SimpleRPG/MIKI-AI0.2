...
import { deterministicCapabilityEvolutionService } from '../../capability/services/deterministicCapabilityEvolutionService';
    ...
    const runVariant = async (variant: { systemPrompt: string }): Promise<string> => {
      ...
      try {
        const stream = isNativeReady
          ? deterministicRuntimeService.streamDeterministicChat(messages, { temperature: 0.7, max_tokens: 400 })
          : deterministicRuntimeService.streamChat(messages, { temperature: 0.7, max_tokens: 400 });
