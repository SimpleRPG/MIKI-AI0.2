...
async function generateContentWithFallback(
  ...
  throw lastError || new Error('外部教師APIの呼び出しに失敗しました。');
}

/** src/miki 配下の .ts/.tsx を再帰列挙する（node_modules等は除外） */
function listMikiServiceFiles(): string[] {
  const root = path.join(process.cwd(), 'src', 'miki');
  const out: string[] = [];
...
app.get('/api/master-spec', (req, res) => {
  const specPath = path.join(process.cwd(), 'docs/MIKI_AI_18_DOMAIN_DESIGN_PHILOSOPHY.txt');
  if (fs.existsSync(specPath)) {
    res.setHeader('Content-Disposition', 'attachment; filename="MIKI_AI_18_DOMAIN_DESIGN_PHILOSOPHY.txt"');
...
interface SelfCodeDecisionReceipt {
...
const SELF_CODE_DECISION_TTL_MS = 5 * 60 * 1000;
const selfCodeDecisionReceipts = new Map<string, SelfCodeDecisionReceipt>();

app.post('/api/self-code/request-write-decision', async (req, res) => {
  try {
    const { chapterNumber, artifactSha256, verificationEvidenceId } = req.body || {};
    if (typeof chapterNumber !== 'number' || typeof artifactSha256 !== 'string' || !/^[a-f0-9]{64}$/i.test(artifactSha256) || typeof verificationEvidenceId !== 'string' || !verificationEvidenceId) {
      return res.status(400).json({ success: false, error: 'VERIFIED_ARTIFACT_REQUIRED' });
    }
    const requestId = `self-code-write-${crypto.randomUUID()}`;
    const result = await coreTaskIngressService.submit({
      kind: 'SYSTEM_TASK',
      goal: `検証済み自己改善成果物を第${chapterNumber}章モジュールとして保存する可否を決定する`,
      source: 'selfDevelopment',
      payload: { requestId, operation: 'WRITE_MODULE', chapterNumber, artifactSha256, verificationEvidenceId },
      maxCycles: 18,
    });
    if (result.task.status !== 'COMPLETED') {
      return res.status(409).json({ success: false, error: 'CORE_DECISION_NOT_COMPLETED', taskId: result.task.taskId, status: result.task.status });
    }
    const coreDecisionId = `decision-${crypto.randomUUID()}`;
    selfCodeDecisionReceipts.set(coreDecisionId, { coreDecisionId, taskId: result.task.taskId, operation: 'WRITE_MODULE', artifactSha256: artifactSha256.toLowerCase(), verificationEvidenceId, expiresAt: Date.now() + SELF_CODE_DECISION_TTL_MS, consumed: false });
    return res.json({ success: true, coreDecisionId, taskId: result.task.taskId, expiresAt: Date.now() + SELF_CODE_DECISION_TTL_MS });
  } catch (error: any) {
    return res.status(500).json({ success: false, error: error?.message || 'CORE_DECISION_REQUEST_FAILED' });
  }
});

app.post('/api/self-code/write-module', (req, res) => {
  try {
    const { chapterNumber, title, filename, code, verificationEvidenceId, artifactSha256, coreDecisionId } = req.body;
    if (typeof chapterNumber !== 'number' || !code || typeof code !== 'string') {
      return res.status(400).json({ success: false, error: '章番号またはコード内容が不正です。' });
    }
    if (typeof verificationEvidenceId !== 'string' || !verificationEvidenceId || typeof artifactSha256 !== 'string' || !/^[a-f0-9]{64}$/i.test(artifactSha256) || typeof coreDecisionId !== 'string' || !coreDecisionId) {
      return res.status(409).json({ success: false, written: false, error: 'CORE_DECISION_AND_VERIFICATION_REQUIRED' });
    }
    const receipt = selfCodeDecisionReceipts.get(coreDecisionId);
    if (!receipt || receipt.consumed || receipt.expiresAt < Date.now() || receipt.operation !== 'WRITE_MODULE' || receipt.artifactSha256 !== artifactSha256.toLowerCase() || receipt.verificationEvidenceId !== verificationEvidenceId) {
      return res.status(409).json({ success: false, written: false, error: 'CORE_DECISION_RECEIPT_INVALID' });
    }
    const calculatedArtifactSha256 = crypto.createHash('sha256').update(code.replace(/\r\n?/g, '\n'), 'utf8').digest('hex');
    if (calculatedArtifactSha256 !== artifactSha256.toLowerCase()) {
      return res.status(409).json({ success: false, written: false, error: 'ARTIFACT_SHA256_MISMATCH' });
    }
...
app.post('/api/self-code/dry-run-verify', (req, res) => {
  try {
    ...
    const artifactSha256 = crypto.createHash('sha256').update(code.replace(/\r?\n/g, '\n'), 'utf8').digest('hex');
    const verificationEvidenceId = valid ? `dry-run-${crypto.randomUUID()}` : '';
    return res.json({
      valid,
      verificationEvidenceId,
      artifactSha256,
...
app.post('/api/self-code/stage-proposal', (_req, res) => {
  return res.status(409).json({
    success: false,
    staged: false,
    status: 'BLOCKED',
    error: 'CORE_INGRESS_REQUIRED',
    operation: 'STAGE_PROPOSAL',
    message: '提案の物理ステージングはcore DecisionCommand統合完了まで停止されています。',
  });
});
...
app.post('/api/self-code/unit-test-run', (req, res) => {
  try {
    ...
    // 現行tsconfigを読み込み、候補コードの構文・型オプション整合を先に確認する。
    const tsconfigPath = path.join(process.cwd(), 'tsconfig.json');
    const configFile = ts.readConfigFile(tsconfigPath, ts.sys.readFile);
    const parsedConfig = configFile.error
      ? undefined
      : ts.parseJsonConfigFileContent(configFile.config, ts.sys, process.cwd());
    const compilerOptions = parsedConfig?.options || { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022 };
    const transpile = ts.transpileModule(codeText, {
      compilerOptions: { ...compilerOptions, module: ts.ModuleKind.CommonJS, noEmit: false },
      reportDiagnostics: true,
      fileName: `${moduleName}.ts`,
    });
    const typeDiagnostics = (transpile.diagnostics || []).map((diagnostic) =>
      ts.flattenDiagnosticMessageText(diagnostic.messageText, '\n')
    );
    const typecheckPassed = Boolean(parsedConfig) && typeDiagnostics.length === 0;
    ...
    try {
      const jsCode = transpile.outputText;
    ...
    const test1Passed = typecheckPassed && !executionError && exportedNames.length > 0;
    const tests: Array<{ id: string; title: string; assertion: string; passed: boolean; durationMs: number; note: string }> = [
      {
        id: 'test-0',
        title: '【現行tsconfig整合確認】候補コードが現行CompilerOptionsで診断なしとなること',
        assertion: 'expect(typeDiagnostics).toHaveLength(0)',
        passed: typecheckPassed,
        durationMs: 0,
        note: parsedConfig ? (typeDiagnostics.join(' | ') || '現行tsconfigのCompilerOptionsで診断なし') : 'tsconfig.jsonを読み込めませんでした',
      },
      {
        id: 'test-1',
    ...
    return res.json({
      ...
      structuralHealthy: typecheckPassed && test1Passed && instantiationOk && !executionError,
      tsconfigTypecheck: { passed: typecheckPassed, diagnostics: typeDiagnostics },
...
app.post('/api/self-code/lessons', (req, res) => {
  ...
  if (typeof title !== 'string' || !title.trim() || typeof rule !== 'string' || !rule.trim()) {
    return res.status(400).json({ success: false, error: 'title と rule は必須です' });
  }
  return res.status(409).json({ success: false, status: 'BLOCKED', error: 'CORE_INGRESS_REQUIRED', operation: 'LEARNING_CANDIDATE_CREATE' });
});
...
app.post('/api/self-code/dead-code-scan', (req, res) => {
  try {
    ...
    const totalEligibleFiles = fs.existsSync(modulesDir) ? fs.readdirSync(modulesDir).filter((f) => f.endsWith('.ts')).length : 0;
    const scannedFilesCount = totalEligibleFiles;
    return res.json({
      success: true,
      evidenceClass: 'HEURISTIC_SUSPECT_ONLY',
      scannedFilesCount,
      totalEligibleFiles,
      partial: scannedFilesCount < totalEligibleFiles,
      unscannedFiles: Math.max(0, totalEligibleFiles - scannedFilesCount),
...
// 5. 自然言語 Prompt-to-Patch パッチ生成機
// 実装方針: キーワード固定テンプレは廃止。実ファイル群からLLMが対象を選定し、
// 実際のファイル内容に基づくSEARCH/REPLACE差分を生成する。LLM未接続時は
// 架空の成功を返さず success:false で正直に失敗を伝える (selfCodeApiClient.ts と同じ不変原則)。
app.post('/api/self-code/prompt-to-patch', async (req, res) => {
  ...
  try {
    // 1. 実ファイル一覧を取得し、プロンプトとの単純な字句一致度で候補を絞り込む
    const allFiles = listMikiServiceFiles();
    ...
    const scored = allFiles
      .map((f) => {
        ...
        const score = tokens.reduce((acc, t) => (lowerPath.includes(t) ? acc + 1 : acc), 0);
        return { file: f, score };
      })
      .sort((a, b) => b.score - a.score);
    const shortlist = (scored.some((s) => s.score > 0) ? scored.filter((s) => s.score > 0) : scored).slice(0, 12);

    const fileSnippets = shortlist.map(({ file }) => {
      let content = '';
      try {
      ...
      return { file, snippet: content.slice(0, 1200) };
    });

    const catalogText = fileSnippets
      .map(({ file, snippet }) => `--- FILE: ${file} ---\n${snippet}`)
      .join('\n\n');

    const llmPrompt = `あなたはMIKI-AIの自己コード改善エンジンです。以下のユーザー指示と実ファイル抜粋から、
最も適切な修正対象ファイルを実ファイル一覧の中から1つ選び、実際のコード内容に基づいたAider形式の
SEARCH/REPLACEパッチを生成してください。存在しないファイルやコードを創作してはいけません。
SEARCH側は対象ファイル内に一字一句そのまま存在する連続した行にしてください。

【ユーザー指示】
${prompt}

【実ファイル抜粋（この中からのみ選ぶこと）】
${catalogText || '(該当ファイルなし)'}

必ず以下のJSON形式のみを出力してください（Markdownバッククォート不要、説明文不要）:
{
  "targetFile": "選んだファイルの相対パス（上記一覧に実在するパスそのまま）",
  "targetFeature": "この修正が対象とする機能の短い説明（日本語）",
  "reasoning": "なぜこのファイル・この修正を選んだかの説明（日本語）",
  "searchReplaceDiff": "<<<<<<< SEARCH\\n（対象ファイルに実在する連続行そのまま）\\n=======\\n（修正後のコード）\\n>>>>>>> REPLACE"
}`;

    const apiKeys = extractAllApiKeys(req);
    let llmText = '';
    let lastErrorMsg = '';
    if (apiKeys.length > 0) {
      try {
        ...
        llmText = response?.text?.trim() || '';
      } catch (err: any) {
        lastErrorMsg = err?.message || String(err);
        console.warn('[Prompt-to-Patch] Gemini API notice:', err);
      }
    }
    ...
    if (!llmText) {
      return res.json({
        success: false,
        prompt,
        targetFile: shortlist[0]?.file || '',
        targetFeature: '外部教師API未設定または応答なし',
        reasoning: lastErrorMsg
          ? `外部教師APIでエラーが発生しました: ${lastErrorMsg}`
          : '外部教師API(Gemini)が設定されていないため、パッチ生成を実行できませんでした。設定からAPIキーを設定するか、Non-LLM自己改善サイクルをご利用ください。',
        searchReplaceDiff: '',
        dryRunValid: false,
      });
    }
    ...
    if (!jsonMatch) {
      return res.json({
        ...
        targetFile: '',
        targetFeature: '',
        reasoning: 'モデルの応答がJSON形式ではなかったため、パッチを生成できませんでした。',
        searchReplaceDiff: '',
        dryRunValid: false,
      });
    }
    ...
    try {
      ...
      return res.json({
        ...
        targetFile: '',
        targetFeature: '',
        reasoning: `モデルの応答JSONの解析に失敗しました: ${parseErr?.message || parseErr}`,
        searchReplaceDiff: '',
        dryRunValid: false,
      });
    }

    const targetFile: string = String(parsed.targetFile || '').trim();
    const targetFeature: string = String(parsed.targetFeature || '未分類の改善');
    const reasoning: string = String(parsed.reasoning || '');
    const searchReplaceDiff: string = String(parsed.searchReplaceDiff || '');

    // 2. 実ファイル存在チェック（LLMが幻覚で存在しないパスを返した場合を検出）
    const resolvedPath = targetFile
      ? path.resolve(process.cwd(), targetFile.startsWith('src/') ? targetFile : path.join('src', targetFile))
      : '';
    const fileExists = targetFile ? fs.existsSync(resolvedPath) || fs.existsSync(path.resolve(process.cwd(), targetFile)) : false;
    const actualFilePath = fs.existsSync(resolvedPath) ? resolvedPath : path.resolve(process.cwd(), targetFile);

    // 3. Dry-run検証: SEARCH側ブロックが実ファイルに本当に存在するかを検査（空白差異は許容）
    let dryRunValid = false;
    if (fileExists) {
      const searchBlockMatch = searchReplaceDiff.match(/<<<<<<< SEARCH\n([\s\S]*?)\n=======/);
      const searchBlock = searchBlockMatch ? searchBlockMatch[1] : '';
      if (searchBlock.trim()) {
        try {
          const actualContent = fs.readFileSync(actualFilePath, 'utf-8');
          const normalize = (s: string) => s.replace(/\s+/g, ' ').trim();
          dryRunValid =
            actualContent.includes(searchBlock) || normalize(actualContent).includes(normalize(searchBlock));
        } catch {
          dryRunValid = false;
        }
      }
    ...
    return res.status(dryRunValid ? 200 : 422).json({
      success: dryRunValid,
      status: dryRunValid ? 'VALIDATED' : 'PROPOSED',
      prompt,
      targetFile,
      targetFeature,
      reasoning: fileExists
        ? reasoning
...
app.post('/api/self-code/rollback-snapshot', (_req, res) => {
  return res.status(409).json({
    success: false,
    restored: false,
    status: 'BLOCKED',
    ...
    message: '物理ロールバックはcore DecisionCommand統合完了まで停止されています。',
  });
});
...
app.post('/api/self-code/reflexion', async (req, res) => {
  try {
    ...
    // 1. 外部教師API (Gemini) が利用可能な場合は、反省パッチの生成を試行
    const apiKeys = extractAllApiKeys(req);
    if (apiKeys.length > 0) {
      try {
...
必ず以下のJSON形式のみを出力してください（Markdownバッククォート不要）:
{
  ...
  "confidenceScore": 88
        ...
        console.warn('[Reflexion API] External Gemini execution notice:', geminiErr);
      }
    }

    // 2. 決定論的分析（実引数 failureReason と targetFile に基づく厳格解析）
...
app.get('/api/self-code/complexity-heatmap', async (req, res) => {
  try {
    ...
    const filesToScan = [
      ...
      { path: 'miki/improvement/miki/improvement/miki/improvement/services/selfImprovementSuiteService.ts', category: 'SELF_IMPROVE' },
      { path: 'miki/selfDevelopment/miki/selfDevelopment/miki/selfDevelopment/services/aiderEngineService.ts', category: 'AIDER' },
      { path: 'miki/selfDevelopment/miki/selfDevelopment/miki/selfDevelopment/services/mikiSelfCodingSuperchargerService.ts', category: 'SUPERCHARGER' },
      { path: 'miki/selfAwareness/miki/selfAwareness/miki/selfAwareness/services/mikiCognitiveVitalsService.ts', category: 'VITALS' },
      { path: 'miki/autonomy/miki/autonomy/services/mikiUltraEvolverService.ts', category: 'EVOLVER' },
...
app.post('/api/self-code/big-o-optimize', async (req, res) => {
  try {
    ...
    // 1. 外部教師API (Gemini) が利用可能な場合は、高度なアルゴリズム最適化パッチ生成を試行
    const apiKeys = extractAllApiKeys(req);
    if (apiKeys.length > 0) {
      try {
...
必ず以下のJSON形式のみを出力してください（Markdownバッククォート不要）:
{
  ...
  "patchDiff": "SEARCH/REPLACE形式の差分"
        ...
        console.warn('[Big-O Optimizer API] External Gemini execution notice:', geminiErr);
      }
    }

    // 2. 構造的・決定論的解析（入力コードの構造を実際に解析）
