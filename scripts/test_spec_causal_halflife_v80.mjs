...
const files = [
  'miki/selfDevelopment/services/specContractCompilerService.ts',
  'miki/memory/services/causalMemoryLedgerService.ts',
  'miki/memory/services/knowledgeHalfLifeService.ts',
  'server.ts',
  'miki/execution/services/backgroundWorkerService.ts',
];
let ok=0;
...
const checks=[
 ...
 ['background integration',fs.readFileSync(path.join(root,'miki/execution/services/backgroundWorkerService.ts'),'utf8').includes('Step 6.12.10')],
 ['non-llm boundary retained',fs.readFileSync(path.join(root,'miki/selfDevelopment/services/specContractCompilerService.ts'),'utf8').includes('生成モデルには依存しない')],
];
