...
const resolveMikiPath = (rel) => {
  const p1 = path.join(root, 'src/miki', rel);
  if (fs.existsSync(p1)) return p1;
  return path.join(root, 'miki', rel);
};

const evolution = fs.readFileSync(resolveMikiPath('capability/services/deterministicCapabilityEvolutionService.ts'), 'utf8');
...
const context = fs.readFileSync(resolveMikiPath('strategy/services/contextBudgetEngineService.ts'), 'utf8');
