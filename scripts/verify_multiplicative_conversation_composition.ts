...
async function runVerification() {
  ...
  assert(negGradResult.status === 'REJECTED', 'ステータスがREJECTEDに更新された');
  ...
  assert(posGradResult.success, '反証合格かつ肯定的フィードバックで昇格が承認された');
  assert(posGradResult.status === 'VERIFIED', 'ステータスが正式に「VERIFIED」に昇格した');
  console.log(`  昇格ログ: ${posGradResult.message}`);
  console.log('  昇格ゲート（反証検査および実利用反応）の動作を確認！\n');
