...
export default function App() {
  ...
  const handleStopGeneration = async () => {
    ...
    // Await this: it force-resets the local engine if the model doesn't respond to
    // the interrupt in time, so a frozen generation can't silently block the next message.
  ...
  const handleSendMessage = async (
    ...
    try {
      ...
      if (shouldRunWebGPU && isGpuUsable) {
        ...
        // If WebGPU encountered an error or model not cached: WebGPU mode means "local
        // only" to the user — never silently call the cloud API here, since that spends
        // real Gemini tokens without asking. Just report what went wrong instead.
        if (!webGpuSuccess || accumulated.trim().length === 0) {
          {
            let diagnosticCategory = '端末推論エラー';
            let diagnosticCause = webGpuErrorDetails || '不明なエラーで端末推論が完了しませんでした。';
            let diagnosticTip = '「再送信」をお試しいただくか、「端末ローカルLLM設定」で別のモデルに切り替えてください。クラウドには送信していません（トークン消費0）。';
            ...
            accumulated = `⚠️ 端末ローカル推論が完了しませんでした。\n・原因: ${diagnosticCause}\n・対処: ${diagnosticTip}`;
            tokenCount = 0;
            firstTokenTime = performance.now();
          }
        }
