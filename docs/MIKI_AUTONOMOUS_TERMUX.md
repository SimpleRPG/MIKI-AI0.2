...
npx tsx scripts/miki-autonomous-runner.ts \
  --duration 1h \
  --instruction /path/to/current-work-directive.txt
```
...
作業指示書はリポジトリへ固定保存せず、実行時に `--instruction` で現在の指示書ファイルを指定する。既存の `WorkDirectiveIngestionService` が、目的・対象ファイル・要求・禁止事項・完了条件を抽出する。
...
cd ~/MIKI-AI0.2
git status
git add scripts/miki-autonomous-runner.ts docs/MIKI_AUTONOMOUS_TERMUX.md
