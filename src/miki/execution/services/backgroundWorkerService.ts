...
import { skillsService } from '../../capability/services/skillsService';
import { regressionBenchmarkService } from '../../verification/services/regressionBenchmarkService';
...
import { autonomousGrowthGovernorService } from '../../autonomy/services/autonomousGrowthGovernorService';
import { autonomousRevalidationLoopService } from '../../autonomy/services/autonomousRevalidationLoopService';
import { remediationExecutionCoordinatorService } from './remediationExecutionCoordinatorService';
import { remediationFailureRecoveryService } from '../../safety/services/remediationFailureRecoveryService';
import { virtualExperienceGeneratorService } from '../../experience/services/virtualExperienceGeneratorService';
import { faultInjectionLabService } from '../../verification/services/faultInjectionLabService';
import { specContractCompilerService } from '../../selfDevelopment/services/specContractCompilerService';
import { knowledgeHalfLifeService } from '../../memory/services/knowledgeHalfLifeService';
import { frontierGovernanceService } from '../../strategy/services/frontierGovernanceService';
import { autonomousHardeningService } from '../../autonomy/services/autonomousHardeningService';
...
import { WebMaterialPatternExtractor } from '../../research/services/webMaterialPatternExtractor';
    ...
    try {
        ...
        try {
          ...
          if (homeworks.length > 0) {
            ...
            for (const hw of homeworks.slice(0, 2)) {
              ...
              if (false && '') {
                for await (const chunk of deterministicRuntimeService.streamDeterministicChat(messages, { max_tokens: 256, temperature: 0.5 })) {
                  thoughtResult += chunk;
                }
              } else if (false) {
                for await (const chunk of deterministicRuntimeService.streamChat(messages, { max_tokens: 256, temperature: 0.5 })) {
                  thoughtResult += chunk;
                }
              }
      ...
      try {
        const queuedRequest = selfImprovementIngressService.submit({
          trigger: `background:${triggerSource}`,
          source: 'EXECUTION',
          runType: 'EXECUTION_FAILURE',
          sourceId: logRecord.id,
          priority: 60,
          payload: { triggerSource, backgroundExecutionId: logRecord.id },
        });
        weaknessFound.push(`[自己改善Queue受付] ${queuedRequest.id}`);
      } catch (improvementErr: unknown) {
        const message = improvementErr instanceof Error ? improvementErr.message : String(improvementErr);
        systemLogger.warn('SELF_IMPROVEMENT', `自己改善Queue受付をスキップしました: ${message}`);
      }
