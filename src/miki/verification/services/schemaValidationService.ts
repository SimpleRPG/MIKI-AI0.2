...
export class SchemaValidationService {
  ...
  public migrateUnifiedCognitiveState(input: any): ValidationResult<UnifiedCognitiveStateV2> {
    ...
    const body: Record<string, unknown> = {
      ...
      constraints: Array.isArray(input.constraints) ? input.constraints.filter((x: unknown): x is string => typeof x === 'string' && x.trim().length > 0).map((x: string) => x.trim()) : [],
      ...
      pendingIntentIds: Array.isArray(input.pendingIntentIds) ? [...new Set(input.pendingIntentIds.filter((x: unknown): x is string => typeof x === 'string' && x.trim().length > 0))] : [],
      evidenceIds: Array.isArray(input.evidenceIds) ? [...new Set(input.evidenceIds.filter((x: unknown): x is string => typeof x === 'string' && x.trim().length > 0))] : [],
      unknowns: Array.isArray(input.unknowns) ? input.unknowns.filter((x: unknown): x is string => typeof x === 'string' && x.trim().length > 0) : [],
      capabilityRefs: Array.isArray(input.capabilityRefs) ? input.capabilityRefs.filter((x: unknown): x is string => typeof x === 'string' && x.trim().length > 0) : [],
