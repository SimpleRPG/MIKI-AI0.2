...
export interface ValidationEvidence { kind:ValidationKind; passed:boolean; evidenceId:string; runner:string; candidateHash:string; observedAt:number; simulated:boolean; details:string; command?:string; exitCode?:number; logRef?:string; artifactHash?:string; environmentId?:string; }
...
export class UnifiedValidationCoordinatorService {
 ...
 public record(candidateId:string,candidateHash:string,input:ValidationEvidence):ValidationBundle|undefined {
  ...
  if(!input.runner.trim()||!input.details.trim()||!input.environmentId||!input.artifactHash)return undefined;
  if(input.passed&&(typeof input.exitCode!=='number'||input.exitCode!==0||!input.command||!input.logRef))return undefined;
 ...
 public canPromote(candidateId:string,candidateHash:string):{allowed:boolean;reason:string;bundle?:ValidationBundle}{
  ...
  const inadmissible=bundle.evidence.filter(e=>!e.environmentId||!e.artifactHash||!e.runner.trim()||!e.details.trim()||(e.passed&&(e.exitCode!==0||!e.command||!e.logRef)));
  if(inadmissible.length>0)return {allowed:false,reason:`VALIDATION_EVIDENCE_INADMISSIBLE:${inadmissible.map(e=>e.kind).join(',')}`,bundle:this.clone(bundle)};
