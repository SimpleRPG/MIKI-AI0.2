...
export function initializeUnknownVerificationSubscriber(): void {
  ...
  claimVerificationEventService.subscribe((event) => {
    ...
    const resolutions = unknownResolutionService.list(500).filter((item) => item.claimIds.includes(event.claimId));
    for (const resolution of resolutions) {
      for (const gapId of resolution.capabilityGapIds) capabilityGapService.updateGapStatus(gapId, 'MITIGATED');
