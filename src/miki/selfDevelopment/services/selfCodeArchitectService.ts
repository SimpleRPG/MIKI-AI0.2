...
import { formalConstraintSolverService } from '../../verification/services/formalConstraintSolverService';
...
import { formalProofService, SkillContract } from '../../verification/services/formalProofService';
import { sandboxPermissionService } from '../../safety/services/sandboxPermissionService';
...
import { counterfactualReasoningService } from '../../unknown/services/counterfactualReasoningService';
import { latentIntentMiningService } from '../../unknown/services/latentIntentMiningService';
import { metacognitiveCalibrationService } from '../../selfAwareness/services/metacognitiveCalibrationService';
...
import { apiUrl, getCustomApiHeaders, SERVER_UNAVAILABLE_MESSAGE } from '../../../services/api';
...
export class SelfCodeArchitectService {
  ...
  public generateImprovementProposal(targetChapterNum: number): SelfImprovementProposal {
    ...
    const contract: ChangeContract = {
      ...
      allowedFiles: [
        targetFileHint,
        'miki/learning/services/userProficiencyService.ts',
        'miki/strategy/services/samplingTuningService.ts',
        'miki/conversationStateService.ts',
        'src/types.ts',
      ],
      forbiddenFiles: [
        'miki/safety/services/deterministicRuntimeService.ts:MODEL_WEIGHTS_INTEGRITY_RULE', // モデル重み不変性
  ...
  public async runAutonomousImprovementCycle(targetChapterNum?: number): Promise<{
    ...
    try {
      const stagePayload = {
        ...
        targetFile: proposal.targetFile || 'miki/selfDevelopment/services/selfCodeArchitectService.ts',
  ...
  public async saveModuleFileToServer(
    ...
    try {
      ...
      // 1. 事前Dry-Run検証。未実施・通信失敗・不合格はすべて書込みを停止する。
      ...
      try {
        ...
        if (!verifyRes.ok) {
          systemLogger.warn('SELF_IMPROVEMENT', `[Dry-Run失敗] HTTP ${verifyRes.status} のため書込みを停止しました。`);
          return false;
        }
        const verifyData = await verifyRes.json();
        verificationEvidenceId = typeof verifyData.verificationEvidenceId === 'string' ? verifyData.verificationEvidenceId : '';
        artifactSha256 = typeof verifyData.artifactSha256 === 'string' ? verifyData.artifactSha256 : '';
        if (verifyData.valid !== true || !verificationEvidenceId || !/^[a-f0-9]{64}$/i.test(artifactSha256)) {
          systemLogger.warn('SELF_IMPROVEMENT', `[Dry-Run不合格] 有効な検証EvidenceとSHA-256がないため書込みを停止しました。`);
          return false;
        }
        systemLogger.info('SELF_IMPROVEMENT', `🧪 [Dry-Run合格] 第${chapterNumber}章: Evidence=${verificationEvidenceId}`);
      } catch (dryErr: unknown) {
        const message = dryErr instanceof Error ? dryErr.message : '接続エラー';
        systemLogger.warn('SELF_IMPROVEMENT', `[Dry-Run未完了] ${message} のため書込みを停止しました。`);
        return false;
      }

      const decisionRes = await fetch(apiUrl('/api/self-code/request-write-decision'), {
        method: 'POST',
        headers: getCustomApiHeaders(),
        body: JSON.stringify({ chapterNumber, artifactSha256, verificationEvidenceId }),
      });
      if (!decisionRes.ok) {
        systemLogger.warn('SELF_IMPROVEMENT', `[core Decision拒否] HTTP ${decisionRes.status} のため書込みを停止しました。`);
        return false;
      }
      const decisionData = await decisionRes.json();
      const coreDecisionId = typeof decisionData.coreDecisionId === 'string' ? decisionData.coreDecisionId : '';
      if (!coreDecisionId) {
        systemLogger.warn('SELF_IMPROVEMENT', '[core Decision不在] 書込みを停止しました。');
        return false;
      }
      ...
      if (res.ok) {
        const writeResult = await res.json().catch(() => null);
        if (!writeResult || writeResult.written !== true || typeof writeResult.coreDecisionId !== 'string' || !writeResult.coreDecisionId) {
          systemLogger.warn('SELF_IMPROVEMENT', '[書込み拒否] core Decision receiptを確認できません。');
          return false;
        }
