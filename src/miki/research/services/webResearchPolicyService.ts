...
class WebResearchPolicyService {
  ...
  isSatisfied(progress:WebResearchProgress,policy=this.get()):boolean {const remaining=this.remaining(progress,policy);if(remaining===null||remaining>0)return false;if(policy.requirePrimarySource&&!progress.primarySourceSatisfied)return false;if(policy.requireCounterEvidenceSearch&&!progress.counterEvidenceSearchCompleted)return false;if(progress.conflictingEvidence)return false;return true;}
