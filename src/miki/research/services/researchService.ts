  ...
  public async researchGap(gap: KnowledgeGap, options?: {
    ...
    try {
      ...
      for (let pass = 0; pass < invocationPassLimit; pass++) {
        ...
        const webResearchProgress = { acceptedIndependentSourceCount: new Set(evidence.filter(item => item.status !== "REJECTED" && item.independence_cluster_id).map(item => item.independence_cluster_id)).size, candidateUrlsChecked: results.filter(result => !!result.url).length, renderedPages: readResults.filter(page => page.success && !!page.text.trim()).length, supportingSourceCount: verification.filter(result => result.outcome === "SUPPORTED" || result.outcome === "DEVICE_VERIFIED").length, counterEvidenceSourceCount: queryPlan.status === "READY" && queryPlan.queries[pass]?.intentType === "COUNTEREVIDENCE" ? results.length : 0, primarySourceSatisfied: evidence.some(item => item.status !== "REJECTED" && item.metadata?.research_source_tier_target === "PRIMARY" && item.metadata?.research_source_tier === "PRIMARY"), counterEvidenceSearchCompleted: queryPlan.status === "READY" && queryPlan.queries.slice(0, pass + 1).some(query => query.intentType === "COUNTEREVIDENCE" && evidence.some(item => item.status !== "REJECTED" && !!item.url)), excludedDuplicateCount: 0, rejectedQualityCount: evidence.filter(item => item.status === "REJECTED").length, conflictingEvidence: verification.some(result => result.outcome === "CONTRADICTED") };
