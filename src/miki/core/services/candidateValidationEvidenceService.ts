...
export interface CandidateValidationEvidence { evidenceId:string; workspaceId:string; candidateSha256:string; stage:ValidationStage; passed:boolean; command:string; exitCode:number; startedAt:number; completedAt:number; logRef:string; }
...
class CandidateValidationEvidenceService{
 private rows:CandidateValidationEvidence[]=[];constructor(){this.load();}
 record(input:Omit<CandidateValidationEvidence,'evidenceId'>):CandidateValidationEvidence{const evidenceId=`VE-${input.workspaceId}-${input.stage}-${input.completedAt}`;const row={...input,evidenceId};this.rows=this.rows.filter(x=>!(x.workspaceId===row.workspaceId&&x.stage===row.stage));this.rows.push(row);this.save();return {...row};}
