...
class CandidateValidationRunnerService {
 async run(workspaceId:string):Promise<ValidationRunnerResult>{
  ...
  if(!passed)return this.rollbackFailure(workspaceId,workspace.writeGuardId,reasons.length>0?reasons:['VALIDATION_FAILED']);
  isolatedCandidateWorkspaceService.setStatus(workspaceId,'SHADOW_PASSED');
  return {passed:true,workspaceId,candidateHash:candidateSha256,passedChecks:candidateValidationEvidenceService.list(workspaceId).filter(row=>row.candidateSha256===candidateSha256&&row.passed&&row.exitCode===0).map(row=>row.stage),failedChecks:evaluation.failed,unexecutedChecks:evaluation.missing,evidenceIds:evaluation.evidenceIds,reasons:[]};
 }
