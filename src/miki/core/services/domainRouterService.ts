...
export type DomainCommand =
  ...
  | 'RUN_SELF_IMPROVEMENT' | 'GET_STATUS' | 'FLUSH' | 'VALIDATE_CANDIDATE';
...
export interface NormalizedDomainResult { status:'SUCCEEDED'|'REJECTED'|'FAILED'; summary:string; data?:unknown; evidenceIds:string[]; retryable:boolean; }
...
class DomainRouterService {
  ...
  private finish(envelope:DomainEnvelope,reply:DomainReply):DomainReply{
    const inferred=this.extractEvidenceIds(reply.result);const evidenceIds=[...new Set([...envelope.evidenceIds,...inferred])];const completed:DomainReply={...reply,normalized:reply.normalized||{status:reply.accepted?'SUCCEEDED':'REJECTED',summary:reply.accepted?'Domain command completed':(reply.error||'Domain command rejected'),data:reply.result,evidenceIds,retryable:!reply.accepted&&/MISSING|NOT_FOUND|WAIT|TIMEOUT|RESOURCE/.test(reply.error||'')}};
