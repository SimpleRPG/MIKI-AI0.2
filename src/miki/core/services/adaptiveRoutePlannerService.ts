...
function readNestedResolutionValue(
  ...
  private planAdaptiveImprovement(task:BlackboardTask,input:Record<string,unknown>):PlannedRoute[] {
    ...
    if(latestCandidateGenerationError.startsWith('CODE_COMPOSITION_COMPONENT_UNRESOLVED:')){
      ...
      const resolutionStatus=String(
        readNestedResolutionValue(resolutionValue,'status') || ''
      ).toUpperCase();

      const researchQuestion=String(
        readNestedResolutionValue(resolutionValue,'researchQuestion') ||
        readNestedResolutionValue(resolutionValue,'query') ||
        ''
      ).trim();
