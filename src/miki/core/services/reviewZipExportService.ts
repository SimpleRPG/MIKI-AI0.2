...
class ReviewZipExportService {
 ...
 async create(runId:string,workspaceId:string,options:{mode?:ReviewPackageCreationMode;sourcePackageId?:string;corePlanRevision?:number;operationInstanceId?:string;candidateId?:string;candidateManifestSha256?:string;validationBundleId?:string;taskId?:string;externalReviewQuestions?:string[];learningLineage?:unknown}={}):Promise<ReviewZipCreateResult>{
  ...
  try{
   ...
   const unresolvedChecks=[...validationManifest.stages.filter(item=>item.status==='NOT_RUN'||item.status==='ENVIRONMENT_UNAVAILABLE').map(item=>`${item.status}:${item.stage}`),...(candidateUnknownContext?.candidateNotes??[])];
   ...
   zip.file('validation.json',JSON.stringify({schemaVersion:2,workspaceId,validationManifest,validationSummary:validationManifest,performed:validation.filter(item=>item.status==='PASSED'||item.status==='FAILED'),unperformed:validationManifest.stages.filter(item=>item.status==='NOT_RUN'||item.status==='ENVIRONMENT_UNAVAILABLE'),shadowEvaluation:shadow??null,disclosure:'NOT_RUN and ENVIRONMENT_UNAVAILABLE are never treated as PASS'},null,2));
   ...
   isolatedCandidateWorkspaceService.updateStatus(workspaceId,'EXPORTED');
 ...
 private async rebuildFromSnapshot(record:ReviewPackageLedgerRecord):Promise<ReviewZipCreateResult>{
  ...
  zip.file('baseline_manifest.json',JSON.stringify(base,null,2));zip.file('candidate_manifest.json',JSON.stringify(manifest,null,2));zip.file('package_manifest.json',JSON.stringify({...manifest,packageManifestSha256:record.packageManifestSha256},null,2));zip.file('change_history.json',JSON.stringify({packageId:record.packageId,packageSeriesId:record.packageSeriesId,packageRevision:record.packageRevision,candidateRevision:record.candidateRevision,regeneratedFromFrozenSnapshot:true},null,2));zip.file('issue.json',JSON.stringify({issueId:record.inputs.issueId,runId:record.runId,objective:record.inputs.objective},null,2));const frozenValidationManifest=candidateValidationEvidenceService.evaluate(record.workspaceId,record.candidateManifestSha256);zip.file('validation.json',JSON.stringify({schemaVersion:2,workspaceId:record.workspaceId,validationManifest:frozenValidationManifest,validationSummary:frozenValidationManifest,performed:candidateValidationEvidenceService.list(record.workspaceId).filter(item=>item.status==='PASSED'||item.status==='FAILED'),unperformed:frozenValidationManifest.stages.filter(item=>item.status==='NOT_RUN'||item.status==='ENVIRONMENT_UNAVAILABLE'),shadowEvaluation:shadowEvaluationService.latest(record.workspaceId)??null,disclosure:'NOT_RUN and ENVIRONMENT_UNAVAILABLE are never treated as PASS'},null,2));zip.file('unresolved_checks.json',JSON.stringify({packageId:record.packageId,items:frozenValidationManifest.stages.filter(item=>item.status==='NOT_RUN'||item.status==='ENVIRONMENT_UNAVAILABLE').map(item=>`${item.status}:${item.stage}`)},null,2));zip.file('review_request.json',JSON.stringify({packageId:record.packageId,packageManifestSha256:record.packageManifestSha256,decision:'ACCEPT|REJECT|NEEDS_CHANGES'},null,2));
