...
class ReviewZipExportService {
 ...
 async create(runId:string,workspaceId:string,options:{mode?:ReviewPackageCreationMode;sourcePackageId?:string;corePlanRevision?:number;operationInstanceId?:string;candidateId?:string;candidateManifestSha256?:string;validationBundleId?:string;taskId?:string;externalReviewQuestions?:string[];learningLineage?:unknown}={}):Promise<ReviewZipCreateResult>{
  ...
  try{
   ...
   zip.file('validation.json',JSON.stringify({schemaVersion:2,workspaceId,validationManifest,validationSummary:validationManifest,performed:validation.filter(item=>item.passed).map(item=>({...item,status:'PASSED'})),unperformed:validationManifest.missing.map(stage=>({stage,status:'NOT_RUN'})),shadowEvaluation:shadow??null,disclosure:'NOT_RUN and ENVIRONMENT_UNAVAILABLE are never treated as PASS'},null,2));
