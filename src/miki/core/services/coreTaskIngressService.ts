...
export interface CoreTaskIngressRequest {kind:CoreTaskKind;goal:string;source:MikiDomain;payload?:Record<string,unknown>;maxCycles?:number;}
class CoreTaskIngressService {
 submit(request:CoreTaskIngressRequest):Promise<CoreOrchestrationResult>{
  ...
  return coreOrchestratorService.run(request.goal,request.source,{kind:request.kind,...(request.payload||{})},maxCycles);
 }
