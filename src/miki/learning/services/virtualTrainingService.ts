  ...
  public async runCapabilityEvaluationTrial(
    input:
      ...
      | {
          ...
          observedEvidence?: {
            sameProblemRetestPassed: boolean;
            paraphraseRetestPassed: boolean;
            crossDomainRetestPassed: boolean;
    ...
    const trialId = `trial_${Date.now()}_${String(this.trials.length + 1).padStart(6, '0')}`;
    ...
    const observedEvidence = params.observedEvidence;
    const hasObservedEvidence = Boolean(observedEvidence && observedEvidence.evidenceIds.length > 0);

    // 3〜6. 外部または実行Runnerが記録したEvidenceだけを使用する。
    // Evidence未接続時はfail-closedで全項目を未合格にする。
    const step3_sameProblemRetestPassed = hasObservedEvidence && Boolean(observedEvidence?.sameProblemRetestPassed);
    const step4_paraphraseRetestPassed = hasObservedEvidence && Boolean(observedEvidence?.paraphraseRetestPassed);
    const step5_crossDomainRetestPassed = hasObservedEvidence && Boolean(observedEvidence?.crossDomainRetestPassed);
    const step6_regressionCheckPassed = hasObservedEvidence && Boolean(observedEvidence?.regressionCheckPassed);
    ...
    if (!hasObservedEvidence) {
      ...
      verdictDetails = '❓ 【判定: Evidence待ち】実際の再試験、言い換え、別分野、回帰試験のEvidenceがないため昇格しません。';
    } else if (step3_sameProblemRetestPassed && step4_paraphraseRetestPassed && step5_crossDomainRetestPassed && step6_regressionCheckPassed) {
