...
export class AutonomousGrowthGovernorService {
  ...
  public async runCycle(options?: { allowSelfCodeImprovement?: boolean; signal?: AbortSignal }): Promise<AutonomousGrowthCycleResult> {
    ...
    try {
      ...
      // 4) 自己コード改善はCanonicalな全自動進化パイプラインへ渡す。
      // 旧runAutonomousImprovementCycle()は提案生成・シミュレーションまでで停止するため、
      // Growth Governorから呼ぶと実装・検証・デプロイまで到達しない。
      if (options?.allowSelfCodeImprovement && selfCode.passed) {
        try {
          const queuedRequest = selfImprovementIngressService.submit({
            trigger: `growth-governor:${cycleId}`,
            source: 'SYSTEM',
            runType: 'AUTONOMOUS_DISCOVERY',
            sourceId: cycleId,
            priority: 70,
            payload: { allowSelfCodeImprovement: true, selfCodeComplianceScore: selfCode.complianceScore },
          });
          actions.push(`self-code improvement queued through core ingress: ${queuedRequest.id}`);
        } catch (e: any) {
