...
export interface MemoryItem {
  ...
  lastUsedAt?: number;
  useCount?: number;
  retrievalCount?: number; // 検索候補として取得された回数
  usageCount?: number; // 回答・処理で実際に利用された回数
  executionSuccessCount?: number; // 利用した処理が成功した回数
  validationSuccessCount?: number; // 独立検証に合格した回数
  validationFailureCount?: number; // 独立検証に失敗した回数
  positiveFeedbackEvidenceCount?: number; // 肯定的ユーザーEvidence件数。検証成功とは別
  negativeFeedbackEvidenceCount?: number; // 否定的ユーザーEvidence件数。検証失敗とは別
  correctionEvidenceCount?: number; // 訂正Evidence件数
  independentSuccessContextCount?: number; // 独立文脈で成功した件数
  lastValidatedAt?: number; // 独立検証を最後に実施した日時
...
export interface CoreTrackingInfo {
  requestId: string;
  interactionId?: string;
  runId?: string;
  directiveId?: string;
  status: 'accepted' | 'processing' | 'waiting' | 'completed' | 'failed' | 'blocked' | 'rejected' | 'inconclusive';
  route: string[];
...
export interface ChatMessage {
  ...
  isStreaming?: boolean;
  isError?: boolean;
  // CORE Result 追跡 (CORE 18構成 閉ループ)
  coreTracking?: CoreTrackingInfo;
