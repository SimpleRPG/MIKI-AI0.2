...
export const EvidenceBasedLoopSubView: React.FC = () => {
  ...
  const [coreRuntimeStatus, setCoreRuntimeStatus] = useState('Core受付待ち');
  ...
  const refreshState = () => {
    ...
    const restoredRuntime = typedImprovementUiGatewayService.listRestoredPriorityOneRuntime();
    const latestRuntime = restoredRuntime[restoredRuntime.length - 1];
    if (latestRuntime) {
      const stopReason = latestRuntime.stopReasons[0];
      setCoreRuntimeStatus(stopReason
        ? `${latestRuntime.taskStatus}: ${stopReason}`
        : `${latestRuntime.taskStatus}: ${latestRuntime.allowedActions.join(', ')}`);
    } else {
      setCoreRuntimeStatus('Core受付待ち');
    }
  };
  ...
  const handleRunCanonicalCycle = async () => {
    ...
    try {
      const result = await typedImprovementUiGatewayService.startSpecifiedImprovement('手動UIから自己改善を実行する', 'auto');
      setExecutionMessage(
        `CORE処理: ${result.currentStage} (Task: ${result.taskId || 'N/A'}, CORE: ${result.coreResult?.requestId || 'N/A'})`
      );
      refreshState();
    } catch (err: any) {
      ...
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white p-6 rounded-2xl shadow-xl border border-indigo-500/20">
        ...
        <div className="mt-4 pt-4 border-t border-slate-800/80 grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
          ...
          <div className="flex items-center gap-2 bg-slate-800/60 p-2.5 rounded-xl border border-slate-700/60">
            ...
            <span className="font-semibold text-slate-300">Core復旧状態:</span>
