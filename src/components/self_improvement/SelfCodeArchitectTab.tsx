...
import {
  typedSelfCodeArchitectUiGatewayService,
  type ImprovementRecipe,
} from '../../miki/core/ui/typedSelfCodeArchitectUiGatewayService';
...
const gateway = typedSelfCodeArchitectUiGatewayService;
...
export const SelfCodeArchitectTab: React.FC = () => {
  ...
  const [auditResult, setAuditResult] = useState<SelfCodeAuditResult>(() =>
    gateway.architect.getLatestAudit({ silent: true }) ?? gateway.architect.runSelfCodeAudit({ silent: true })
  );
  const [proposals, setProposals] = useState<SelfImprovementProposal[]>(() =>
    gateway.architect.getProposals()
  );
  ...
  const [activeRecipe, setActiveRecipe] = useState<ImprovementRecipe | null>(() =>
    gateway.architect.getRecipeForChapter(33, { silent: true })
  );
  const [moduleSearch, setModuleSearch] = useState('');

  // ── 指示書 1.6: 承認ゲート待機レコード管理用ステート ──
  const [pendingApprovalRecords, setPendingApprovalRecords] = useState(() =>
    gateway.continuousEvolution.getHistory().filter((r) => r.awaitingApproval)
  );
  ...
  useEffect(() => {
    ...
    const unsubSteps = gateway.continuousEvolution.subscribeSteps((step) => {
    ...
    const unsubState = gateway.continuousEvolution.subscribe((record, isRunning) => {
      if (isRunning) {
  ...
  const completedChapters = gateway.specificationRegistry.filter((c) => c.status === 'COMPLETED');
  const unimplementedChapters = gateway.specificationRegistry.filter((c) => c.status !== 'COMPLETED');
  ...
  const handleRunAudit = () => {
    setIsAuditing(true);
    setTimeout(() => {
      const result = gateway.architect.runSelfCodeAudit();
  ...
  const handleGenerateProposal = (chapterNumber: number) => {
    const proposal = gateway.architect.generateImprovementProposal(chapterNumber);
    setProposals(gateway.architect.getProposals());
  ...
  const handleSimulate = (proposalId: string) => {
    const updated = gateway.architect.simulateProposal(proposalId);
    if (updated) {
      setProposals([...gateway.architect.getProposals()]);
  ...
  const handleApply = async (proposalId: string) => {
    ...
    try {
      const prop = gateway.architect.getProposals().find((p) => p.id === proposalId);
      const success = await gateway.architect.applyProposal(proposalId);
      if (success) {
        setProposals([...gateway.architect.getProposals()]);
        setAuditResult(gateway.architect.getLatestAudit()!);
        const updatedProp = gateway.architect.getProposals().find((p) => p.id === proposalId);
        const methodDesc =
  ...
  const handleRollback = (proposalId: string) => {
    const success = gateway.architect.rollbackProposal(proposalId);
    if (success) {
      setProposals([...gateway.architect.getProposals()]);
      setAuditResult(gateway.architect.getLatestAudit()!);
  ...
  const handleDownloadProposalZip = async (prop: SelfImprovementProposal) => {
    ...
    try {
      ...
      const stagePayload = {
        ...
        targetFile: prop.targetFile || prop.contract?.allowedFiles?.[0] || 'miki/selfDevelopment/services/gateway.architect.ts',
        title: prop.title,
        prompt: prop.prompt,
  ...
  const handleRunTeacherDrift = async () => {
    ...
    try {
      const res = await gateway.teacherDrift.runDriftCheck();
      setDriftResult(res);
  ...
  const handleTestUtteranceAnalysis = () => {
    ...
    gateway.userProficiency.analyzeUserUtterance(testUtterance);
    setProfScores(gateway.userProficiency.getAllScores());
  ...
  const runRealtimeChapterCycle = async (targetChapter: SpecificationChapterMeta) => {
    ...
    try {
      ...
      const updatedAudit = gateway.architect.getLatestAudit()!;
  ...
  const handleAutoImproveCycle = (chapterNum: number) => {
    const chap = gateway.specificationRegistry.find((c) => c.chapterNumber === chapterNum);
    if (chap) {
  ...
  const handleAutonomousMikiImprovement = () => {
    const nextChapter = unimplementedChapters[0] || gateway.specificationRegistry[0];
  ...
  const handleBatchImprovement = async (count?: number) => {
    ...
    const finalAudit = gateway.architect.getLatestAudit()!;
          ...
          <div className="p-3.5 bg-indigo-950/40 border border-indigo-500/30 rounded-2xl space-y-2.5 text-xs">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 text-slate-300">
              ...
              <div className="p-2.5 bg-slate-900/80 border border-slate-800 rounded-xl space-y-1">
                ...
                <p className="text-[11px] leading-relaxed text-slate-400">
                  ・各カード内の<strong>「許可ファイル」</strong>に対象ファイル（例: <span className="font-mono text-emerald-300">gateway.userProficiency.ts</span>）を明示<br />
                      ...
                      <div className="mt-2 p-3 bg-slate-950 border border-slate-800 rounded-xl space-y-1.5 font-mono text-[10px]">
                        <div className="flex items-center justify-between text-slate-400 text-[10px] pb-1 border-b border-slate-800">
                          <span className="flex items-center gap-1 text-slate-200 font-semibold truncate">
                            ...
                            変更対象: {prop.contract.allowedFiles[0] || 'miki/selfDevelopment/services/gateway.architect.ts'}
                          </span>
      ...
      {activeView === 'invariants' && (
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {gateway.architect.checkInvariants().checks.map((inv: InvariantCheckItem) => (
              <div
                key={inv.id}
                ...
                <div className="mt-3 space-y-2">
                  ...
                  <div className="flex justify-between text-xs text-slate-300">
                    ...
                    <span className="font-bold text-emerald-400 font-mono">
                      ×{gateway.teacherDrift.getConfidencePenaltyWeight().toFixed(2)}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs text-slate-300">
                    <span>状態:</span>
              ...
              <div>
                ...
                <div className="mt-3 space-y-2">
                  ...
                  {(() => {
                    ...
                    const advice = gateway.userProficiency.getExplanationAdvice(selectedProfDomain);
                    return (
        ...
        <div className="space-y-4 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 bg-slate-900/80 border border-slate-800 rounded-2xl space-y-3">
              ...
              {(() => {
                const boundaries = gateway.curriculum.getAllBoundaries();
                const skillIrs = gateway.curriculum.getAllSkillIrs();
                return (
            ...
            <div className="p-4 bg-slate-900/80 border border-slate-800 rounded-2xl space-y-3">
              ...
              {(() => {
                const ctx = gateway.proactiveContext.getLatestContext();
                const persona = gateway.proactiveContext.verifyAndRestorePersona();
                return (
            ...
            <div className="p-4 bg-slate-900/80 border border-slate-800 rounded-2xl space-y-3">
              ...
              {(() => {
                const stats = gateway.researchNotes.getStats();
                const latest = gateway.researchNotes.getAllExperiments()[0];
                return (
                  <div className="space-y-2.5 text-xs">
            ...
            <div className="p-4 bg-slate-900/80 border border-slate-800 rounded-2xl space-y-3">
              ...
              {(() => {
                const latest = gateway.debugger.getLatestTrace();
                return (
        ...
        <div className="space-y-6">
          ...
          <div className="p-4 bg-slate-900/80 border border-slate-800 rounded-2xl space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
              {gateway.architect.getArchitectureOverview().map((layer, idx) => (
          ...
          <div className="p-4 bg-slate-900/80 border border-slate-800 rounded-2xl space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {gateway.architect.getModules(moduleSearch).map((mod, idx) => (
