...
export const AutonomousImprovementSettingsPanel: React.FC = () => {
  ...
  const ingestDirective = (text: string) => {
    if (!text.trim()) return;
    const directive = workDirectiveIngestionService.ingestDirectiveText(text, settings.title || undefined);
  ...
  const runLoop = async () => {
    ...
    try {
      ...
      while (!stopRef.current && (limit === Number.POSITIVE_INFINITY || Date.now() - started < limit)) {
        cycle += 1;
        try {
          const result = await selfImprovementControllerService.runOnce('app-autonomous-settings');
          setMessage(`サイクル ${cycle}: [${result.decision.action}] ${result.result || '完了'}`);
        } catch (err: any) {
          setMessage(`サイクル ${cycle} エラー: ${err?.message || String(err)}`);
        }
        if (stopRef.current) break;
        await new Promise(resolve => setTimeout(resolve, Math.max(5, settings.intervalSeconds) * 1000));
      }
      setMessage(stopRef.current ? '停止しました' : '設定した時間が終了しました');
    } finally {
      setRunning(false);
      setStartedAt(null);
      stopRef.current = false;
