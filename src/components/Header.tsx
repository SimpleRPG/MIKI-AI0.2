...
interface HeaderProps {
  activeTab: 'preview' | 'code' | 'improvement' | 'github';
  setActiveTab: (tab: 'preview' | 'code' | 'improvement' | 'github') => void;
  ...
  return (
    <header className="h-13 sm:h-14 bg-slate-950/95 backdrop-blur-md border-b border-slate-800/80 px-3 sm:px-4 flex items-center justify-between gap-3 select-none z-30 shrink-0">
      ...
      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
        ...
        {/* 18-Structure Status Badge */}
        <button
          onClick={() => setActiveTab('improvement')}
          className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-medium border transition-all shadow-xs cursor-pointer ${
            activeTab === 'improvement'
              ? 'bg-indigo-950/90 border-indigo-500/60 text-indigo-200'
              : 'bg-slate-900/90 border-slate-800 hover:border-indigo-500/50 text-slate-200 hover:text-white'
          }`}
          title="【MIKI 18構成アーキテクチャ】第18統括層(CORE)がUI入出力を一元管理し、17分類を自律連携（クリックで管理画面を開く）"
        >
          <Cpu className="w-3.5 h-3.5 text-indigo-400" />
          <span className="hidden sm:inline font-bold">18構成 (CORE+17)</span>
          <span className="sm:hidden font-bold">CORE</span>
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-medium border transition-all shadow-xs ${
            activeTab === 'improvement'
              ? 'bg-indigo-950/80 border-indigo-500/60 text-indigo-300 font-bold'
              : 'bg-slate-900/90 border-slate-800 hover:border-indigo-500/50 text-slate-300 hover:text-indigo-300 hover:bg-slate-800'
          }`}
          title="自律改善 統合ホームを開く (状態・作業指示・実行・キュー・検証・履歴)"
        >
          <Sparkles className="w-3.5 h-3.5 text-indigo-400" />
          <span className="hidden sm:inline font-bold">自律改善</span>
          <span className="w-1.5 h-1.5 rounded-full bg-indigo-400/80" />
        </button>
      </div>
