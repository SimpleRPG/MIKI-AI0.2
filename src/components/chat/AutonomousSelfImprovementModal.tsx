...
export const AutonomousSelfImprovementModal: React.FC<AutonomousSelfImprovementModalProps> = ({
  ...
  const handleRunBatch = async (count: number = 3) => {
    try {
      ...
      const result = await typedImprovementUiGatewayService.startSpecifiedImprovement(`上位${count}章の連続バッチ自己改善`, 'batch');
      setNotice(`CORE自己改善Task: ${result.currentStage} (Task: ${result.taskId || 'N/A'}, CORE: ${result.coreResult?.requestId || 'N/A'})`);
      refreshBacklog();
    } catch (err: any) {
  ...
  const handleRunSingleCycle = async (override?: { chapterNumber?: number; prompt?: string }) => {
    try {
      ...
      const result = await typedImprovementUiGatewayService.startSpecifiedImprovement(override?.prompt || '自律自己改善を実行する', target?.chapterNumber != null ? `chapter-${target.chapterNumber}` : 'auto');
      setNotice(`CORE自己改善Task: ${result.currentStage} (Task: ${result.taskId || 'N/A'}, CORE: ${result.coreResult?.requestId || 'N/A'})`);
    } catch (err: any) {
