...
import {
  ...
  workflowSynthesisService,
  experienceRouterService,
  autonomousEvolutionService,
  mikiSelfCodingSuperchargerService,
  autonomousContinuousEvolutionService,
  userProficiencyService,
  codeSkeletonService,
  conversationBranchService,
  liveConversationRepairService,
  whyAnswerInspectorService,
  conversationTaskboardService,
  proactiveContextOsService,
  userFeedbackGovernanceService,
} from '../miki/core/ui/typedChatUiGatewayService';
import type {
...
export const ChatPanel: React.FC<ChatPanelProps> = ({
  ...
  const handleFeedback = (msg: ChatMessage, type: 'good' | 'bad', reason?: string) => {
    ...
    userFeedbackGovernanceService.record({
      subjectType: 'CHAT_RESPONSE', subjectId: msg.id,
      polarity: type === 'good' ? 'POSITIVE' : 'NEGATIVE',
      userText: reason,
      reason: 'ユーザー評価は最終判定ではなく追加検証が必要なEvidence',
      source: 'THUMBS',
    });
                    ...
                    {msg.externalLlmDiagnostic && (() => {
                      ...
                      return (
                        <button
                          ...
                          title="外部LLMのStage A〜E遅延細分化、実測timings、KVキャッシュ判定を展開"
                        >
                          <Terminal className="w-3 h-3 text-indigo-400" />
                          <span>
                            外部AI (TTFT: {diag.observedTtftMs}ms
...
${diag.comparisonWithPrevious ? `【連続実行TTFT比較判定】\n${diag.comparisonWithPrevious.explanation}` : ''}`;
                ...
                >
                  ...
                  {/* CORE 18構成 追跡 (実データ: STEP 10) */}
                  {msg.coreTracking && (
                    <div className="mt-3 p-3 bg-slate-950/95 border border-indigo-500/40 rounded-xl space-y-2 text-xs shadow-md">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1.5 font-bold text-slate-200">
                          <Network className="w-3.5 h-3.5 text-indigo-400" />
                          <span>CORE 18構成 追跡 (実データ)</span>
                        </div>
                        <span
                          className={`px-2 py-0.5 rounded-md text-[10px] font-bold ${
                            msg.coreTracking.status === 'completed'
                              ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40'
                              : msg.coreTracking.status === 'processing'
                              ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/40 animate-pulse'
                              : msg.coreTracking.status === 'accepted'
                              ? 'bg-sky-500/20 text-sky-300 border border-sky-500/40'
                              : msg.coreTracking.status === 'waiting'
                              ? 'bg-amber-500/20 text-amber-300 border border-amber-500/40'
                              : msg.coreTracking.status === 'failed'
                              ? 'bg-rose-500/20 text-rose-300 border border-rose-500/40'
                              : msg.coreTracking.status === 'rejected'
                              ? 'bg-zinc-500/20 text-zinc-300 border border-zinc-500/40'
                              : 'bg-slate-500/20 text-slate-300 border border-slate-500/40'
                          }`}
                        >
                          {msg.coreTracking.status === 'completed' && '✅ 実行済み (Completed)'}
                          {msg.coreTracking.status === 'processing' && '⚡ 処理中 (Processing)'}
                          {msg.coreTracking.status === 'accepted' && '📥 受付済み (Accepted)'}
                          {msg.coreTracking.status === 'waiting' && '⏳ 待機中 (Waiting)'}
                          {msg.coreTracking.status === 'failed' && '❌ 失敗 (Failed)'}
                          {msg.coreTracking.status === 'rejected' && '⛔ 却下 (Rejected)'}
                          {msg.coreTracking.status === 'inconclusive' && '❓ 判定不能 (Inconclusive)'}
                          {msg.coreTracking.status === 'blocked' && '🛑 ブロック (Blocked)'}
                        </span>
                      </div>

                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 text-[10.5px] font-mono bg-black/50 p-2.5 rounded-lg border border-slate-800">
                        <div>
                          <span className="text-slate-500 font-sans">Request ID: </span>
                          <span className="text-indigo-300 font-semibold">{msg.coreTracking.requestId}</span>
                        </div>
                        {msg.coreTracking.interactionId && (
                          <div>
                            <span className="text-slate-500 font-sans">Interaction ID: </span>
                            <span className="text-sky-300">{msg.coreTracking.interactionId}</span>
                          </div>
                        )}
                        <div>
                          <span className="text-slate-500 font-sans">Run ID: </span>
                          <span className="text-slate-300">{msg.coreTracking.runId || 'なし (通常対話)'}</span>
                        </div>
                        {msg.coreTracking.directiveId && (
                          <div>
                            <span className="text-slate-500 font-sans">Directive ID: </span>
                            <span className="text-amber-300">{msg.coreTracking.directiveId}</span>
                          </div>
                        )}
                        <div className="sm:col-span-2">
                          <span className="text-slate-500 font-sans">Route: </span>
                          <span className="text-emerald-300">{msg.coreTracking.route?.length > 0 ? msg.coreTracking.route.join(' → ') : 'core'}</span>
                        </div>
                        <div className="sm:col-span-2">
                          <span className="text-slate-500 font-sans">Processed Categories: </span>
                          <span className="text-slate-300">[{msg.coreTracking.processedCategories?.join(', ') || 'none'}]</span>
                        </div>
                        {msg.coreTracking.error && (
                          <div className="sm:col-span-2 text-rose-400">
                            <span className="font-sans">Error: </span>
                            <span>{msg.coreTracking.error}</span>
                          </div>
                        )}
                        <div className="sm:col-span-2 flex justify-between text-[9.5px] text-slate-500 pt-1 border-t border-slate-800/80">
                          <span>最終更新: {new Date(msg.coreTracking.updatedAt).toLocaleTimeString()}</span>
                          <span className="text-indigo-400/80">CORE Result リアルタイム同期</span>
                        </div>
                      </div>
                    </div>
                  )}
      ...
      <div className="p-2 sm:p-3 bg-slate-900 border-t border-slate-800 shrink-0">
        ...
        <div className="mt-1 px-1 flex items-center justify-between text-[10px] text-slate-500 select-none">
          <div className="flex items-center gap-1.5 text-emerald-400/90 truncate">
            ...
            <span className="truncate">CORE統括・決定論的実行（高精度・端末保護）</span>
          </div>
        </div>
      </div>
