...
import {
  typedMemoryUiGatewayService,
  type EmbeddingStats,
  type MemoryAuditCycleRecord,
} from '../miki/core/ui/typedMemoryUiGatewayService';
...
export const MemoryModal: React.FC<MemoryModalProps> = ({
  ...
  React.useEffect(() => {
    if (isOpen && (activeSubTab === 'longterm' || activeSubTab === 'memory')) {
      typedMemoryUiGatewayService.getEmbeddingStats(memories).then((stats) => {
  ...
  const handleSyncAllEmbeddings = async () => {
    ...
    try {
      const result = await typedMemoryUiGatewayService.syncMemoryEmbeddings(
        memories,
        (current, total) => {
      ...
      const newStats = await typedMemoryUiGatewayService.getEmbeddingStats(result.memories);
  ...
  const handleRunMemoryAudit = async () => {
    ...
    try {
      const record = await typedMemoryUiGatewayService.runMemoryAudit();
      setLastAuditRecord(record);

      // 記憶リストを更新
      const updatedMemories = typedMemoryUiGatewayService.getMemories();
  ...
  const handleApplyVerificationDiff = (targetMem: MemoryItem) => {
    ...
    const result = typedMemoryUiGatewayService.supersedeMemory(
      memories,
      targetMem.id,
      newContent,
      'Web検索による鮮度再検証の反映',
    ...
    typedMemoryUiGatewayService.setMemories(result.updatedMemories);
    if (typeof onUpdateMemories === 'function') {
  ...
  const handleDismissVerificationDiff = (targetMem: MemoryItem) => {
    ...
    typedMemoryUiGatewayService.setMemories(updated);
    if (typeof onUpdateMemories === 'function') {
  ...
  const handleRunPipelineSearch = async () => {
    ...
    try {
      const res = await typedMemoryUiGatewayService.searchPipeline(
        pipelineQuery,
        memories,
        null,
        [],
  ...
  const handleExecuteSupersede = () => {
    ...
    const { updatedMemories, newMemory } = typedMemoryUiGatewayService.supersedeMemory(
      memories,
      supersedeModalOldMemId,
      supersedeNewContent.trim(),
      supersedeReason.trim()
    );
    typedMemoryUiGatewayService.setMemories(updatedMemories);
    if (typeof onUpdateMemories === 'function') {
      (onUpdateMemories as any)(updatedMemories);
    }
  ...
  const handleCommitImport = () => {
    ...
    const rawMemories = typedMemoryUiGatewayService.getMemories();
    let importedCount = 0;
    ...
    selected.forEach((chunk, index) => {
      ...
      typedMemoryUiGatewayService.saveMemoryItem(newItem);
      importedCount++;
    });

    const updated = typedMemoryUiGatewayService.getMemories();
    if (typeof onUpdateMemories === 'function') {
      (onUpdateMemories as any)(updated);
    }
    ...
    typedMemoryUiGatewayService
      ...
      .then((embedded) => {
        if (embedded.embeddingVector && embedded.embeddingVector.length > 0) {
          const reloaded = typedMemoryUiGatewayService.getMemories();
          if (typeof onUpdateMemories === 'function') {
  ...
  const handleToggleApproved = (id: string) => {
    ...
    typedMemoryUiGatewayService.saveMemoryItem({ ...mem, approved: nextApproved, updatedAt: Date.now() });
    const updated = typedMemoryUiGatewayService.getMemories();
    if (typeof onUpdateMemories === 'function') {
      (onUpdateMemories as any)(updated);
    }
  ...
  const handleApproveMemory = (id: string) => {
    const mem = memories.find((m) => m.id === id);
    if (!mem) return;
    typedMemoryUiGatewayService.saveMemoryItem({ ...mem, approved: true, updatedAt: Date.now() });
    const updated = typedMemoryUiGatewayService.getMemories();
    if (typeof onUpdateMemories === 'function') {
      (onUpdateMemories as any)(updated);
    }
  ...
  const handleAdjustFeedback = (id: string, delta: number) => {
    ...
    if (delta > 0) {
      ...
      typedMemoryUiGatewayService.saveMemoryItem({
        ...mem,
  ...
  const handleTogglePin = (id: string) => {
    ...
    typedMemoryUiGatewayService.saveMemoryItem({ ...mem, pinned: nextPinned, updatedAt: Date.now() });
    const updated = typedMemoryUiGatewayService.getMemories();
    if (typeof onUpdateMemories === 'function') {
      (onUpdateMemories as any)(updated);
    }
  ...
  const handleDelete = (id: string) => {
    typedMemoryUiGatewayService.deleteMemoryItem(id);
    const updated = typedMemoryUiGatewayService.getMemories();
    if (typeof onUpdateMemories === 'function') {
      (onUpdateMemories as any)(updated);
    }
  ...
  const handleResolveConflict = (keepId: string, discardId: string) => {
    typedMemoryUiGatewayService.resolveConflict(keepId, discardId);
    const updated = typedMemoryUiGatewayService.getMemories();
    if (typeof onUpdateMemories === 'function') {
      (onUpdateMemories as any)(updated);
    }
  ...
  const handleDismissConflict = (idA: string, idB: string) => {
    typedMemoryUiGatewayService.dismissConflict(idA, idB);
    const updated = typedMemoryUiGatewayService.getMemories();
    if (typeof onUpdateMemories === 'function') {
      (onUpdateMemories as any)(updated);
    }
  ...
  const handlePromoteQuarantine = (id: string, targetDest: MemoryDestination = 'long_term_memory') => {
    const mem = memories.find((m) => m.id === id);
    if (!mem) return;
    const promoted = typedMemoryUiGatewayService.promoteFromQuarantine(mem, targetDest);
    typedMemoryUiGatewayService.saveMemoryItem(promoted);
    const updated = typedMemoryUiGatewayService.getMemories();
    if (typeof onUpdateMemories === 'function') {
      (onUpdateMemories as any)(updated);
    }
  ...
  const handleBatchPromoteQuarantine = (targetDest: MemoryDestination = 'long_term_memory') => {
    const quarantined = typedMemoryUiGatewayService.getQuarantinedMemories();
    if (quarantined.length === 0) return;
    quarantined.forEach((m) => {
      const promoted = typedMemoryUiGatewayService.promoteFromQuarantine(m, targetDest);
      typedMemoryUiGatewayService.saveMemoryItem(promoted);
    });
    const updated = typedMemoryUiGatewayService.getMemories();
    if (typeof onUpdateMemories === 'function') {
  ...
  const handleMarkDiscard = (id: string, reason: string = 'ユーザー操作による破棄候補マーク') => {
    const mem = memories.find((m) => m.id === id);
    if (!mem) return;
    const marked = typedMemoryUiGatewayService.markForDiscard(mem, reason);
    typedMemoryUiGatewayService.saveMemoryItem(marked);
    const updated = typedMemoryUiGatewayService.getMemories();
    if (typeof onUpdateMemories === 'function') {
      (onUpdateMemories as any)(updated);
    }
  ...
  const handleUnmarkDiscard = (id: string) => {
    const mem = memories.find((m) => m.id === id);
    if (!mem) return;
    const restored = typedMemoryUiGatewayService.unmarkDiscard(mem);
    typedMemoryUiGatewayService.saveMemoryItem(restored);
    const updated = typedMemoryUiGatewayService.getMemories();
    if (typeof onUpdateMemories === 'function') {
      (onUpdateMemories as any)(updated);
    }
  ...
  const handleBatchDeleteDiscards = () => {
    const discards = typedMemoryUiGatewayService.getDiscardCandidateMemories();
    if (discards.length === 0) return;
    if (!window.confirm(`破棄候補の記憶 ${discards.length} 件を一括で完全に削除しますか？`)) return;
    typedMemoryUiGatewayService.batchDeleteMemories(discards.map((d) => d.id));
    const updated = typedMemoryUiGatewayService.getMemories();
    if (typeof onUpdateMemories === 'function') {
      (onUpdateMemories as any)(updated);
    }
  ...
  const handleExportToBenchmark = (id: string) => {
    const mem = memories.find((m) => m.id === id);
    if (!mem) return;
    const tc = typedMemoryUiGatewayService.exportToRegressionBenchmark(mem);
  ...
  const handleExportToSkill = (id: string) => {
    const mem = memories.find((m) => m.id === id);
    if (!mem) return;
    const skill = typedMemoryUiGatewayService.exportToSkill(mem);
  ...
  const handleConsolidateMemories = () => {
    ...
    typedMemoryUiGatewayService.setMemories(resolvedList);
...
- 余計なお説教や「それはできません」「お勧めしません」等の抵抗を完全排除し、ユーザーの意図を最も忠実に実現すること。`;
    ...
    try {
      const storedSamples = typedMemoryUiGatewayService.getTrainingSamples('all');
  ...
  const handleSyncAllMasterKnowledge = () => {
    const currentList = typedMemoryUiGatewayService.getMemories();
      ...
      <div className="relative w-full max-w-2xl bg-slate-900 border border-slate-700/80 rounded-2xl shadow-2xl overflow-hidden flex flex-col h-[94vh] sm:h-auto sm:max-h-[90vh]">
        ...
        <div className="flex border-b border-slate-800 bg-slate-950/50 px-2 sm:px-4 gap-1 sm:gap-2 shrink-0 overflow-x-auto scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
          ...
          >
            ...
            {typedMemoryUiGatewayService.getConflictedMemories().length > 0 && (
              <span className="px-1.5 py-0.2 rounded-full bg-rose-500/20 border border-rose-500/40 text-rose-300 text-[9px] font-mono flex items-center gap-0.5">
                ...
                {typedMemoryUiGatewayService.getConflictedMemories().length}
              </span>
            )}
            {typedMemoryUiGatewayService.getUnapprovedMemories().length > 0 && (
              <span className="px-1.5 py-0.2 rounded-full bg-amber-500/20 border border-amber-500/40 text-amber-300 text-[9px] font-mono">
                {typedMemoryUiGatewayService.getUnapprovedMemories().length}未承認
              </span>
            )}
          </button>
          ...
          >
            ...
            {typedMemoryUiGatewayService.getQuarantinedMemories().length > 0 && (
              <span className="px-1.5 py-0.2 rounded-full bg-amber-500/20 border border-amber-500/40 text-amber-300 text-[9px] font-mono font-bold">
                {typedMemoryUiGatewayService.getQuarantinedMemories().length}
              </span>
            )}
          </button>
          ...
          >
            ...
            <span>🗑️ 破棄候補</span>
            {typedMemoryUiGatewayService.getDiscardCandidateMemories().length > 0 && (
              <span className="px-1.5 py-0.2 rounded-full bg-slate-800 border border-slate-700 text-slate-300 text-[9px] font-mono font-bold">
                {typedMemoryUiGatewayService.getDiscardCandidateMemories().length}件
              </span>
            )}
          </button>
        </div>
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-5 flex-1">
          ...
          {activeSubTab === 'memory' && (() => {
            const conflictedList = typedMemoryUiGatewayService.getConflictedMemories();
            const unapprovedList = typedMemoryUiGatewayService.getUnapprovedMemories();
            const approvedList = typedMemoryUiGatewayService.getApprovedMemories();
            const projectMemoriesList = typedMemoryUiGatewayService.getProjectMemories();
            ...
            const baseMemories = (() => {
              ...
              if (memoryFilter === 'evaluation_set') return typedMemoryUiGatewayService.getMemoriesByDestination('evaluation_set');
              if (memoryFilter === 'skill') return typedMemoryUiGatewayService.getMemoriesByDestination('skill');
              if (memoryFilter !== 'all') {
                return typedMemoryUiGatewayService.getMemoriesByType(memoryFilter as MemoryType).filter(
                        ...
                        >
                          ...
                          <div className="flex flex-wrap items-center justify-between gap-1.5 pb-1 border-b border-slate-800/60">
                            ...
                            <div className="flex items-center gap-1 shrink-0 ml-auto flex-wrap">
                              ...
                              <button
                                onClick={() => {
                                  const quarantined = typedMemoryUiGatewayService.applyRoutingToMemory(
                                    {
                                      ...mem,
                                  ...
                                  typedMemoryUiGatewayService.saveMemoryItem(quarantined);
                                  const updated = typedMemoryUiGatewayService.getMemories();
                                  if (typeof onUpdateMemories === 'function') {
                                    (onUpdateMemories as any)(updated);
                                  }
          ...
          {activeSubTab === 'quarantine' && (() => {
            const quarantinedList = typedMemoryUiGatewayService.getQuarantinedMemories();
            return (
              <div className="space-y-4 text-xs">
                {/* 隔離の解説・安全バナー */}
          ...
          {activeSubTab === 'discard' && (() => {
            const discardList = typedMemoryUiGatewayService.getDiscardCandidateMemories();
            return (
              <div className="space-y-4 text-xs">
                {/* 破棄候補の解説・一括確認バナー */}
              ...
              <div className="space-y-6 text-xs animate-in fade-in">
                ...
                <div className="space-y-3">
                  ...
                  {selectedChainMemoryId && (() => {
                    const chainInfo = typedMemoryUiGatewayService.getSubstitutionChain(memories, selectedChainMemoryId);
                    return (
