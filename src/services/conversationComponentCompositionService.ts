...
import { formalConstraintSolverService, BinaryConstraint } from './formalConstraintSolverService';
import { wordAssociationGraphService, RelatednessEvaluation } from './wordAssociationGraphService';
import { falsificationService } from './falsificationService';
...
export type ComposedResponseStatus = 'CANDIDATE' | 'VERIFIED' | 'REJECTED';
  ...
  public graduateComposedResponse(
    ...
    if (!record) {
      return {
        ...
        status: 'REJECTED',
        falsificationPassed: false,
        message: `指定された合成応答 ${responseId} が見つかりません`,
      };
    }

    // 1. falsificationService による反証検査
    ...
    const falsificationPassed = falsification.passed && falsification.checks.every(c => c.status !== 'fail');
    record.falsificationPassed = falsificationPassed;

    // 2. 反証失敗、またはユーザー反応がネガティブなら却下
    if (!falsificationPassed || userFeedback === 'NEGATIVE') {
      record.status = 'REJECTED';
      record.verificationLog = `昇格却下: 反証判定=${falsificationPassed ? 'pass' : 'fail'} (スコア: ${falsification.falsificationScore}), ユーザー反応=${userFeedback}`;
      this.saveToStorage();
      return {
        success: false,
        status: 'REJECTED',
        falsificationPassed,
        message: `昇格条件を満たしませんでした (反証スコア: ${falsification.falsificationScore}, フィードバック: ${userFeedback})`,
      };
    }

    // 3. 両方のゲートを通過した場合のみ VERIFIED に昇格
    record.userFeedbackReceived = true;
    record.status = 'VERIFIED';
    record.verifiedAt = Date.now();
    record.verificationLog = `正式昇格達成: 反証テスト全項目合格 (pass) かつ ユーザー肯定的フィードバック (${userFeedback}) 受領`;
    this.saveToStorage();

    systemLogger.info(
      'SELF_IMPROVEMENT',
      `🏆 [合成応答VERIFIED昇格] ${responseId} が反証と実利用フィードバックを経てVERIFIEDへ昇格しました`
    );
    ...
    return {
      ...
      message: `${responseId} は反証検査に合格し、ユーザー確認を経て正式にVERIFIEDへ昇格しました`,
    };
  }
