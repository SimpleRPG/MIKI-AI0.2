...
export const SelfCodeArchitectTab: React.FC = () => {
  ...
  const runRealtimeChapterCycle = async (targetChapter: SpecificationChapterMeta) => {
    ...
    try {
      ...
      const coverageNote = tddRes.coverage
        ? `行網羅率: ${tddRes.coverage.lines}%, 分岐: ${tddRes.coverage.branches}%`
        : '実行ベース判定 (行網羅率は未計測)';
      addLiveLog(`🧪 [TDD ${tddRes.passedCount}/${tddRes.totalCount} PASSED] (${coverageNote})`, tddRes.allPassed ? 'success' : 'warn');
    } catch {
